class ReportPackagesMonthController < ApplicationController
  def index
  ...
  def show
    ...
    @packages = Package.find_by_sql('select * from packages p join hours h on p.hour_id = h.id
                                   where MONTH(h.hour) = ' + month + ' and YEAR(h.hour) = ' + year)
  end
end
